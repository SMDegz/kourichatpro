...
class ContentGenerator:
    ...
    def _get_prompt_content(self, prompt_type: str, avatar_name: str, user_id: str, max_rounds: int = 15) -> tuple:
        ...
        try:
            ...
            # 读取提示词
            prompt_path = os.path.join(project_root, "data", "base", "prompts", f"{prompt_type}.md")
    ...
    def _generate_content(self, content_type: str, avatar_name: str, user_id: str, max_rounds: int = 15, save_to_file: bool = True) -> str:
        """
        ...
        try:
            ...
            if save_to_file:
                ...
                try:
                    ...
                    logger.info(f"已生成{avatar_name}{content_type_info[content_type]['format_name']} 用户: {user_id} 并保存至: {file_path}")
                except Exception as e:
    ...
    def _generate_content_wrapper(self, content_type: str, avatar_name: str, user_id: str, max_rounds: int, save_to_file: bool = True) -> str:
        """
        生成内容的通用包装方法
