...
def save_phone():
    ...
    try:
        ...
        # 验证数据有效性
        if not client_name or not phone_numbers:
            return jsonify({"status": "error", "message": "客户名称和电话号码不能为空"}), 400
            ...
            try:
                ...
                for phone in phone_numbers:
                    ...
                    record = {
                        ...
                        'hidephone': phone[:3] + '****' + phone[7:],  # 假设hidephone和realphone使用相同的值
                        'realphone': phone,
                        'status': 1,  # 默认状态为1
                        'spare1':str,
                    ...
                    # 插入单条记录
                    db_handler.insert_data(conn, 'CoreCmsPhoneCompare', record)
                    
                return jsonify({"status": "success", "message": f"成功保存 {len(phone_numbers)} 条记录"})
        ...
        logger.error(f"保存客户电话关联失败: {str(e)}")
        return jsonify({"status": "error", "message": "服务器处理请求时出错"}), 500
